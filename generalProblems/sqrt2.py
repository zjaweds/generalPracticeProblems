...
for i in range(10000):
 #Generating random numbers between 1 and 2 and assigning it to x as x the sqrt(2) is assumed to lie in 1 to 2
    x=random.random()+1
 #Checking whether point with coordinate x^2 lies between 1 and 2 or not
    if(x*x-2<=0):
#For each point lying in between 1 and 2 we increment n
 #to have count of points lying in that interval
        n=n+1;
# Result for the square root we have count of the points in interval 1 and sqrt(2)
